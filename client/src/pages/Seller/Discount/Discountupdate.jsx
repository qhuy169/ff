...
const Discountupdate = () => {
    ...
    const { id } = useParams();  // Get the discount ID from the URL
    ...
    // Fetch the discount data by ID when the component is mounted
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await discountService.putDiscount(id, formDataToSend); // API to update the discount
            if (response.status === 200) {
                alert('Discount updated successfully!');
                navigate('/seller/discounts');  // Redirect to the discount list page
            }
        } catch (error) {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
            <h2 className="text-3xl font-bold text-center text-gray-800 mb-6">Edit Discount</h2>
